...
__device__ __forceinline__ void mat4(u64 *x){
    __uint128_t S=(__uint128_t)x[0]; S+=(__uint128_t)x[1]; S+=(__uint128_t)x[2]; S+=(__uint128_t)x[3];
    u64 o[4];
    #pragma unroll
    for (int i=0;i<4;i++){ __uint128_t v=S; v+=(__uint128_t)x[i]; __uint128_t t=(__uint128_t)x[(i+1)&3]; v+=t; v+=t; o[i]=gf_red(v); }
    #pragma unroll
...
__device__ __forceinline__ void ext_lin(u64 *s, const u64 *rc){
    ...
    u64 sum[4];
    #pragma unroll
    for (int i=0;i<4;i++){ __uint128_t a=(__uint128_t)s[i]; a+=(__uint128_t)s[4+i]; a+=(__uint128_t)s[8+i]; sum[i]=gf_red(a); }
    #pragma unroll
