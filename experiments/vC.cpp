...
__device__ __forceinline__ u64 gf_add(u64 a, u64 b){
    // LAZY: fold the 2^64 carry (s-=P is exactly +(2^32-1) mod 2^64) but do NOT canonicalise.
    // Values stay in [0,2^64) and remain correct mod p; the final squeeze does one conditional
    // subtract, which suffices because 2^64-P = 2^32-1 < P.
    u64 s = a + b; if (s < a) s -= P; return s;
}
...
__device__ __forceinline__ u64 gf_red(__uint128_t x){
    ...
    u64 a = lo - (u64)hh;  if (lo < (u64)hh) a += P;          // lo - hi_hi
...
__device__ __forceinline__ void mat4(u64 *x){
    __uint128_t S = (__uint128_t)x[0]+(__uint128_t)x[1]+(__uint128_t)x[2]+(__uint128_t)x[3];
    u64 o[4];
    #pragma unroll
    for (int i=0;i<4;i++){ __uint128_t t=(__uint128_t)x[(i+1)&3]; o[i]=gf_red(S+(__uint128_t)x[i]+t+t); }
    #pragma unroll
