...
// Accumulator reduce for x < 2^96 (all linear-layer sums qualify: <= 12*2^64).
// hi fits in 32 bits so the high-half (2^96 == -1) term is identically zero and its
// borrow branch disappears: two conditionals instead of three.
__device__ __forceinline__ u64 gf_red_s(__uint128_t x){
    u64 lo=(u64)x; u32 hl=(u32)(u64)(x>>64);
    u64 b=((u64)hl<<32)-(u64)hl;
    u64 r=lo+b; if(r<lo) r+=0xFFFFFFFFULL;
    if(r>=P) r-=P;
    return r;
}
__device__ __forceinline__ u64 gf_add(u64 a, u64 b){ return gf_red_s((__uint128_t)a + (__uint128_t)b); }
...
__device__ __forceinline__ void ext_lin(u64 *s, const u64 *rc){
    ...
    u64 sum[4];
    #pragma unroll
    for (int i=0;i<4;i++) sum[i]=gf_red_s((__uint128_t)s[i]+(__uint128_t)s[4+i]+(__uint128_t)s[8+i]);
    #pragma unroll
    for (int i=0;i<12;i++){ __uint128_t z=(__uint128_t)s[i]+(__uint128_t)sum[i&3]; if(rc) z+=(__uint128_t)rc[i]; s[i]=gf_red_s(z); }
}
...
__device__ __forceinline__ void int_lin(u64 *s, u64 rc0){
    ...
    u64 sum = gf_red_s(acc);
    #pragma unroll
    for (int i=0;i<12;i++){ __uint128_t v=(__uint128_t)gf_mul(s[i],D_MAT_DIAG[i])+(__uint128_t)sum; if(i==0) v+=(__uint128_t)rc0; s[i]=gf_red_s(v); }
}
