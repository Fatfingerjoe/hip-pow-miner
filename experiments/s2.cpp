...
// Single-conditional reduce, valid for x < 2^96 (validated in ft3.cpp; worst site hi=7).
...
__device__ __forceinline__ void sparse_state(const u64 *pre, u64 idx, u64 *s){
    ...
    // Collapse the whole column reconstruction into ONE reduce per lane.
    // Columns are <=6, so pre[i] + 6*y3 + 6*y7 < 13*2^64 -> hi < 16, inside gf_red_s bound.
    #pragma unroll
    for (int i=0;i<12;i++){
        __uint128_t v = (__uint128_t)pre[i];
        v += (__uint128_t)y3 * (__uint128_t)D_MECOL3[i];
        v += (__uint128_t)y7 * (__uint128_t)D_MECOL7[i];
        s[i] = gf_red_s(v);
    }
}
