...
static __device__ __forceinline__ void apply_mat4(u64 *x) {
    ...
    u64 x0d = gf_addL(x[0], x[0]);
    u64 x2d = gf_addL(x[2], x[2]);
    x[3] = gf_addL(t01233, x0d);
    x[1] = gf_addL(t01123, x2d);
    x[0] = gf_addL(t01123, t01);
    x[2] = gf_addL(t01233, t23);
}
static __device__ __forceinline__ void external_linear(u64 *s, const u64 *rc) {
    u64 y[12];
    #pragma unroll
    ...
    #pragma unroll
    for (int i=0;i<12;i++) {
        u64 z = gf_addL(y[i], sum[i&3]);
        if (rc) z = gf_addL(z, rc[i]);
        s[i] = z;
    }
}
