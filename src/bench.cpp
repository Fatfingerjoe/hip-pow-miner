...
int main(int argc, char **argv) {
    ...
    if (strcmp(mode, "perm0")==0) {
        ...
        perm0_kernel<<<1,1>>>(d_z,d_o);
        hipDeviceSynchronize();
        hipMemcpy(got,d_o,12*sizeof(u64),hipMemcpyDeviceToHost);
        u64 want[12]={
            0xc9bc9432e1686884ULL,0x03ecbab0dcdd2189ULL,0x5e7ac885b3dc1215ULL,
            0x6ac07513801d191fULL,0xca5c593fb184dcfcULL,0x414dec5f3e455287ULL,
            0x1a17df170127ae41ULL,0xe7e592bd0af9b0a5ULL,0xc71a9b27edc66a4cULL,
            0x2728671759ac43c2ULL,0xb9969c20f7f672f9ULL,0xc5140b586823b92fULL
        };
        int fail=0;
        for(int i=0;i<12;i++) {
