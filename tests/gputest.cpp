...
__global__ void k(const u64* in, u64* outB, u64* outU, int n){
  int t=blockIdx.x*blockDim.x+threadIdx.x; if(t>=n) return;
  u64 a[4],b[4];
  for(int i=0;i<4;i++){a[i]=in[4*t+i]; b[i]=in[4*t+i];}
  mat4_base(a); mat4_u128(b);
  for(int i=0;i<4;i++){outB[4*t+i]=a[i]; outU[4*t+i]=b[i];}
}
int main(){
  const int N=6;
  ...
  // CPU reference
  u64 cB[N*4], cU[N*4];
  for(int t=0;t<N;t++){u64 a[4],b[4];
    for(int i=0;i<4;i++){a[i]=h[4*t+i]; b[i]=h[4*t+i];}
    mat4_base(a); mat4_u128(b);
    for(int i=0;i<4;i++){cB[4*t+i]=a[i]; cU[4*t+i]=b[i];}}
  u64 *d,*oB,*oU; hipMalloc(&d,sizeof h); hipMalloc(&oB,sizeof h); hipMalloc(&oU,sizeof h);
  hipMemcpy(d,h,sizeof h,hipMemcpyHostToDevice);
  hipLaunchKernelGGL(k,dim3(1),dim3(N),0,0,d,oB,oU,N);
  ...
  printf("%s\n", bad? "MISMATCHES FOUND":"GPU and CPU agree on BOTH forms, and the two forms agree");
  return 0;
}
